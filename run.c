...
static int isposnumber(const char * str)
{
  const char * p;
  int res;
...
static unsigned parse_number_argument(int * i, int argc, char ** argv)
{
  unsigned res;

  if(argv[*i][2])
    {
      if(isposnumber(argv[*i] + 2)) res = (unsigned) atoi(argv[*i] + 2);
      else goto ARGUMENT_IS_MISSING;
    }
  else
  if(*i + 1 < argc && isposnumber(argv[*i + 1]))
    {
...
static void print_long_command_line_option(FILE * file, char * str)
{
  const char * p;

  for(p = str; *p && *p != ' '; p++) fputc(*p, file);
}

/*------------------------------------------------------------------------*/

static unsigned parse_number_rhs(char * str)
{
  unsigned res;
  char * p;
...
static FILE * open_log(const char * name, const char * option)
{
  FILE * res;
...
static unsigned get_physical_mb()
{
  unsigned res;
  long tmp;
...
{
  if(s == SIGUSR1) caught_usr1_signal = 1;
}
...
{
  int i, j, res, silent, child, status, s, ok;
  ...
  for(i = 1; i < argc; i++)
    {
	  ...
	  if(strstr(argv[i], "--time-limit=") == argv[i])
	    {
	  ...
	  if(strstr(argv[i], "--space-limit=") == argv[i])
	    {
	  ...
	  if(argv[i][1] == 'o')
	    {
	      if(argv[i][2]) log = open_log(argv[i] + 2, "-o");
	      else log = open_log(i + 1 >= argc ? 0 : argv[++i], "-o");
	    }
	  else
	  ...
	  if(WIFSIGNALED(status))
	    {
	      switch((s = WTERMSIG(status)))
	        {
		  case SIGXFSZ: ok = OUT_OF_MEMORY; break;
		  case SIGXCPU: ok = OUT_OF_TIME; break;
		  case SIGSEGV: ok = SEGMENTATION_FAULT; break;
		  case SIGBUS: ok = BUS_ERROR; break;
		  default: ok = OTHER_SIGNAL; break;
		}
  ...
  if(caught_usr1_signal) ok = EXEC_FAILED;
  ...
  if(!silent)
    {
      ...
      switch(ok)
        {
	  case OK: fputs("ok", log); break; 
	  case OUT_OF_TIME: fputs("out of time", log); break;
	  case OUT_OF_MEMORY: fputs("out of memory", log); break;
	  case SEGMENTATION_FAULT: fputs("segmentation fault", log); break;
	  case BUS_ERROR: fputs("bus error", log); break;
	  case FORK_FAILED: fputs("fork failed", log); break;
	  case INTERNAL_ERROR: fputs("internal error", log); break;
	  case EXEC_FAILED: fputs("execvp failed", log); break;
	  default: fprintf(log, "signal(%d)\n", s); break;
	}
      fputc('\n', log);
...
#         ifdef INCLUDE_SYSTEM_TIME
	    ((double)u.ru_stime.tv_sec) + 10e-7*((double)u.ru_stime.tv_usec) +
#         endif
	    ((double)u.ru_utime.tv_sec) + 10e-7*((double)u.ru_utime.tv_usec));
	  
	  fprintf(log, "[run] space:\t\t%.1f MB\n",
	    u.ru_maxrss / 1024.0 / 1024.0);
	}
      else fprintf(log, "[run] could not get resource usage\n");
      fflush(log);
    }
